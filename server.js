...
// Main HTML page - FIXED TEMPLATE LITERAL
app.get('/', (req, res) => {
    ...
    <script>
        ...
        function setupHeroSlider(items) {
            ...
            if (!items || items.length === 0) {
                slider.innerHTML = '<div class="slide active" style="background: linear-gradient(135deg, var(--accent-blue), var(--accent-purple));"><div class="slide-content"><h2 class="slide-title">Welcome to CLOUD.MOVIES</h2><p class="slide-description">Stream thousands of movies and TV series in stunning quality</p><div class="slide-meta"><div class="meta-item"><i class="fas fa-film"></i><span>Premium Streaming</span></div><div class="meta-item"><i class="fas fa-hd"></i><span>HD Quality</span></div><div class="meta-item"><i class="fas fa-globe"></i><span>Global Content</span></div></div><div class="slide-actions"><button class="slide-btn watch-btn" onclick="performSearch(\'movie\')"><i class="fas fa-play"></i><span>Browse Movies</span></button><button class="slide-btn info-btn" onclick="performSearch(\'tv\')"><i class="fas fa-info-circle"></i><span>Explore Series</span></button></div></div></div>';
                return;
            }
            
            // Create slides
            slider.innerHTML = items.map((item, index) => {
                const backgroundImage = item.cover || item.poster || 'https://images.unsplash.com/photo-1536440136628-849c177e76a1?auto=format&fit=crop&w=1600&q=80';
                return '<div class="slide ' + (index === 0 ? 'active' : '') + '" style="background-image: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url(\'' + backgroundImage + '\');"><div class="slide-content"><h2 class="slide-title">' + item.title + '</h2><p class="slide-description">' + (item.description || 'An exciting movie available for streaming now') + '</p><div class="slide-meta">' + (item.year ? '<div class="meta-item"><i class="fas fa-calendar"></i><span>' + item.year + '</span></div>' : '') + (item.type ? '<div class="meta-item"><i class="fas fa-' + (item.type === 'tv' ? 'tv' : 'film') + '"></i><span>' + (item.type === 'tv' ? 'TV Series' : 'Movie') + '</span></div>' : '') + (item.rating ? '<div class="meta-item"><i class="fas fa-star"></i><span>' + item.rating + '/10</span></div>' : '') + '</div><div class="slide-actions"><button class="slide-btn watch-btn" onclick="showDetails(\'' + item.id + '\')"><i class="fas fa-play"></i><span>Watch Now</span></button><button class="slide-btn info-btn" onclick="showDetails(\'' + item.id + '\')"><i class="fas fa-info-circle"></i><span>More Info</span></button></div></div></div>';
            }).join('');
            
            // Add slider controls
            slider.innerHTML += '<div class="slider-controls"><button class="slider-btn prev-btn" onclick="prevSlide()"><i class="fas fa-chevron-left"></i></button><button class="slider-btn next-btn" onclick="nextSlide()"><i class="fas fa-chevron-right"></i></button></div><div class="slider-dots">' + items.map((_, index) => '<div class="slider-dot ' + (index === 0 ? 'active' : '') + '" onclick="goToSlide(' + index + ')"></div>').join('') + '</div>';
        ...
        function setupQuickCategories() {
            ...
            categoriesContainer.innerHTML = quickCategories.map(cat => '<div class="quick-category" data-search="' + cat.search + '"><i class="' + cat.icon + '"></i><span>' + cat.name + '</span></div>').join('');
        ...
        function displayContentInGrid(grid, items) {
            if (!items || items.length === 0) {
                grid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; padding: 3rem;"><div style="font-size: 4rem; color: var(--text-secondary); opacity: 0.3; margin-bottom: 1rem;"><i class="fas fa-film"></i></div><p style="color: var(--text-secondary);">No content found</p></div>';
                return;
            }
            
            grid.innerHTML = items.map(item => {
                const ratingBadge = item.rating ? '<div class="rating-badge"><i class="fas fa-star"></i><span>' + item.rating + '</span></div>' : '';
                ...
                const genresHtml = item.genre && item.genre.length > 0 ? '<div class="card-genres">' + item.genre.slice(0, 2).map(g => '<span class="genre-tag">' + g + '</span>').join('') + '</div>' : '';
                
                return '<div class="content-card" data-id="' + item.id + '">' + ratingBadge + '<img src="' + posterUrl + '" alt="' + item.title + '" class="card-poster" onerror="this.src=\'https://images.unsplash.com/photo-1536440136628-849c177e76a1?auto=format&fit=crop&w=500&q=80\'"><div class="card-info"><h3 class="card-title" title="' + item.title + '">' + item.title + '</h3><div class="card-meta"><span>' + (item.year || 'N/A') + '</span><span><i class="fas fa-' + (item.type === 'tv' ? 'tv' : 'film') + '"></i> ' + (item.type === 'tv' ? 'Series' : 'Movie') + '</span></div>' + genresHtml + '</div></div>';
            }).join('');
        ...
        function displayDetails(movie) {
            ...
            if (hasEpisodes) {
                ...
                episodesHtml = '<div class="episode-section"><h3><i class="fas fa-tv"></i> Seasons & Episodes</h3><p style="color: var(--text-secondary); margin-bottom: 1.5rem;">Select multiple episodes for batch streaming or download</p><div class="season-selector" id="season-selector">' + seasons.map(season => {
                    const seasonEpisodes = movie.episodes.filter(ep => ep.season === season);
                    const episodeCount = seasonEpisodes.length;
                    return '<div class="season-card" data-season="' + season + '"><div style="font-size: 2rem; margin-bottom: 0.5rem; color: var(--accent-blue);"><i class="fas fa-play-circle"></i></div><div style="font-weight: 600;">Season ' + season + '</div><div style="font-size: 0.9rem; color: var(--text-secondary);">' + episodeCount + ' episode' + (episodeCount !== 1 ? 's' : '') + '</div></div>';
                }).join('') + '</div><div id="episode-container" style="display: none;"><div class="batch-controls"><div><button id="select-all-btn" class="select-all-btn"><i class="fas fa-check-double"></i> Select All</button></div><div class="batch-actions"><button id="batch-play-btn" class="batch-btn batch-play" disabled><i class="fas fa-play-circle"></i> Play Selected</button><button id="batch-download-btn" class="batch-btn batch-download" disabled><i class="fas fa-download"></i> Download</button></div></div><div class="episode-grid" id="episode-grid"></div></div></div>';
            }
            
            // Build cast HTML if available
            let castHtml = '';
            if (movie.cast && movie.cast.length > 0) {
                castHtml = '<div class="cast-section"><h3><i class="fas fa-users"></i> Cast & Crew</h3><div class="cast-grid">' + movie.cast.slice(0, 6).map(person => {
                    const avatarUrl = person.avatar || 'https://images.unsplash.com/photo-1494790108755-2616b612b786?auto=format&fit=crop&w=200&q=80';
                    return '<div class="cast-card"><img src="' + avatarUrl + '" alt="' + person.name + '" class="cast-avatar" onerror="this.src=\'https://images.unsplash.com/photo-1494790108755-2616b612b786?auto=format&fit=crop&w=200&q=80\'"><div style="margin-top: 0.5rem;"><strong>' + person.name + '</strong><div style="font-size: 0.9rem; color: var(--text-secondary); margin-top: 0.25rem;">' + (person.character || 'Actor') + '</div></div></div>';
                }).join('') + '</div></div>';
            }
            
            // Build quality selector
            const qualitySelector = '<div class="quality-selector"><h3><i class="fas fa-hd"></i> Select Quality</h3><div class="quality-grid"><div class="quality-card active" data-quality="360p"><div style="font-size: 1.5rem; margin-bottom: 0.5rem; color: var(--accent-blue);"><i class="fas fa-tv"></i></div><div style="font-weight: 600;">360p</div><div style="font-size: 0.9rem; color: var(--text-secondary);">Good</div></div><div class="quality-card" data-quality="480p"><div style="font-size: 1.5rem; margin-bottom: 0.5rem; color: var(--accent-blue);"><i class="fas fa-tv"></i></div><div style="font-weight: 600;">480p</div><div style="font-size: 0.9rem; color: var(--text-secondary);">Better</div></div><div class="quality-card" data-quality="720p"><div style="font-size: 1.5rem; margin-bottom: 0.5rem; color: var(--accent-blue);"><i class="fas fa-hd"></i></div><div style="font-weight: 600;">720p</div><div style="font-size: 0.9rem; color: var(--text-secondary);">HD</div></div></div></div>';
            
            // Build action buttons
            const trailerBtn = movie.trailer ? '<button class="action-btn trailer-btn" id="play-trailer"><i class="fas fa-play"></i><span>Watch Trailer</span></button>' : '';
            const actionButtons = '<div class="action-buttons"><button class="action-btn stream-btn" id="play-single"><i class="fas fa-play-circle"></i><span>' + (isTV ? 'Play Episode' : 'Stream Now') + '</span></button><button class="action-btn download-btn" id="download-single"><i class="fas fa-download"></i><span>' + (isTV ? 'Download Episode' : 'Download') + '</span></button>' + trailerBtn + '</div>';
            
            // Build video container
            const videoContainer = '<div id="video-container" class="video-container" style="display: none; margin-top: 3rem;"><video id="video-player" controls style="width: 100%; height: 100%;">Your browser does not support the video tag.</video></div>';
            
            // Build main content
            const posterUrl = movie.poster || movie.cover || 'https://images.unsplash.com/photo-1536440136628-849c177e76a1?auto=format&fit=crop&w=500&q=80';
            const ratingBadge = movie.rating ? '<div style="background: var(--primary-gradient); color: white; padding: 8px 16px; border-radius: 25px; font-weight: 600; display: flex; align-items: center; gap: 5px;"><i class="fas fa-star"></i><span>' + movie.rating + '/10</span></div>' : '';
            const countryBadge = movie.country ? '<div style="background: rgba(255,255,255,0.1); color: white; padding: 8px 16px; border-radius: 25px; font-weight: 500;"><i class="fas fa-globe"></i><span>' + movie.country + '</span></div>' : '';
            const genreBadges = movie.genre && movie.genre.length > 0 ? movie.genre.map(g => '<span style="background: rgba(102, 126, 234, 0.15); padding: 8px 16px; border-radius: 20px; border: 1px solid rgba(102, 126, 234, 0.3); color: var(--accent-blue); font-weight: 500;">' + g + '</span>').join('') : '';
            const releaseDateInfo = movie.releaseDate ? '<div><div style="font-size: 0.9rem; color: var(--text-secondary); margin-bottom: 0.5rem;">Release Date</div><div style="font-weight: 600; display: flex; align-items: center; gap: 10px;"><i class="fas fa-calendar"></i><span>' + movie.releaseDate + '</span></div></div>' : '';
            const durationInfo = movie.duration ? '<div><div style="font-size: 0.9rem; color: var(--text-secondary); margin-bottom: 0.5rem;">Duration</div><div style="font-weight: 600; display: flex; align-items: center; gap: 10px;"><i class="fas fa-clock"></i><span>' + Math.floor(movie.duration / 60) + ' minutes</span></div></div>' : '';
            
            modalBody.innerHTML = '<div class="detail-content"><div style="display: grid; grid-template-columns: 1fr 2fr; gap: 3rem; margin-bottom: 3rem;"><div><img src="' + posterUrl + '" alt="' + movie.title + '" style="width: 100%; border-radius: 20px; border: 2px solid var(--accent-blue); box-shadow: 0 20px 40px rgba(102, 126, 234, 0.3);" onerror="this.src=\'https://images.unsplash.com/photo-1536440136628-849c177e76a1?auto=format&fit=crop&w=500&q=80\'"></div><div><div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1.5rem; flex-wrap: wrap;">' + ratingBadge + '<div style="background: rgba(255,255,255,0.1); color: white; padding: 8px 16px; border-radius: 25px; font-weight: 500;"><i class="fas fa-' + (isTV ? 'tv' : 'film') + '"></i><span>' + (isTV ? 'TV Series' : 'Movie') + '</span></div>' + countryBadge + '</div><div style="display: flex; gap: 1rem; margin-bottom: 1.5rem; flex-wrap: wrap;">' + genreBadges + '</div><p style="margin-bottom: 2rem; line-height: 1.8; color: var(--text-secondary); font-size: 1.1rem;">' + (movie.description || 'No description available.') + '</p><div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1.5rem; margin-bottom: 2rem;">' + releaseDateInfo + durationInfo + '</div></div></div>' + castHtml + episodesHtml + qualitySelector + actionButtons + videoContainer + '</div>';
                ...
                seasonSelector.querySelectorAll('.season-card').forEach(card => {
                    card.addEventListener('click', function() {
                        ...
                        episodeGrid.innerHTML = seasonEpisodes.map(ep => {
                            const episodeId = 'ep-' + selectedSeason + '-' + ep.episode;
                            return '<div class="episode-item"><input type="checkbox" id="' + episodeId + '" class="episode-checkbox" data-season="' + selectedSeason + '" data-episode="' + ep.episode + '" value="s' + selectedSeason + 'e' + ep.episode + '"><label for="' + episodeId + '" class="episode-label"><div style="font-size: 1.2rem; margin-bottom: 0.5rem;"><i class="fas fa-play"></i></div><div style="font-weight: 600;">Episode ' + ep.episode + '</div></label></div>';
                        }).join('');
        ...
        function updatePagination(pagination) {
            ...
            if (currentPage > 1) {
                html += '<button onclick="changePage(' + (currentPage - 1) + ')" style="padding: 12px 24px; background: var(--primary-gradient); border: none; border-radius: 10px; color: white; cursor: pointer; display: flex; align-items: center; gap: 8px; font-weight: 600; transition: all 0.3s ease;"><i class="fas fa-chevron-left"></i> Previous</button>';
            }
            
            html += '<div style="padding: 12px 24px; background: rgba(255,255,255,0.1); border-radius: 10px; font-weight: 600; backdrop-filter: blur(10px);">Page ' + currentPage + ' of ' + totalPages + '</div>';
            
            if (currentPage < totalPages) {
                html += '<button onclick="changePage(' + (currentPage + 1) + ')" style="padding: 12px 24px; background: var(--primary-gradient); border: none; border-radius: 10px; color: white; cursor: pointer; display: flex; align-items: center; gap: 8px; font-weight: 600; transition: all 0.3s ease;">Next <i class="fas fa-chevron-right"></i></button>';
            }
        ...
        function showSkeletonLoading() {
            const grid = document.getElementById('content-grid');
            grid.innerHTML = '<div class="skeleton skeleton-card"></div><div class="skeleton skeleton-card"></div><div class="skeleton skeleton-card"></div><div class="skeleton skeleton-card"></div><div class="skeleton skeleton-card"></div><div class="skeleton skeleton-card"></div>';
        }

        // Show modal loading
        function showModalLoading() {
            const modalBody = document.getElementById('modal-body');
            modalBody.innerHTML = '<div style="text-align: center; padding: 4rem;"><div class="loading-ring" style="margin: 0 auto 2rem;"></div><div style="color: var(--text-secondary); font-size: 1.1rem;">Loading details...</div></div>';
        }

        // Show error message
        function showError(message) {
            const grid = document.getElementById('content-grid');
            grid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; padding: 4rem;"><div style="font-size: 4rem; color: var(--danger); margin-bottom: 1rem;"><i class="fas fa-exclamation-triangle"></i></div><h3 style="color: var(--text-primary); margin-bottom: 1rem;">Oops!</h3><p style="color: var(--text-secondary);">' + message + '</p><button onclick="window.location.reload()" style="margin-top: 2rem; padding: 12px 24px; background: var(--primary-gradient); border: none; border-radius: 10px; color: white; cursor: pointer; font-weight: 600; transition: all 0.3s ease;"><i class="fas fa-redo"></i> Try Again</button></div>';
        }

        // Show modal error
        function showModalError(message) {
            const modalBody = document.getElementById('modal-body');
            modalBody.innerHTML = '<div style="text-align: center; padding: 4rem;"><div style="font-size: 4rem; color: var(--danger); margin-bottom: 1rem;"><i class="fas fa-exclamation-triangle"></i></div><h3 style="color: var(--text-primary); margin-bottom: 1rem;">Something went wrong</h3><p style="color: var(--text-secondary); margin-bottom: 2rem;">' + message + '</p><button onclick="closeModal()" style="padding: 12px 24px; background: var(--primary-gradient); border: none; border-radius: 10px; color: white; cursor: pointer; font-weight: 600;"><i class="fas fa-times"></i> Close</button></div>';
        }
        ...
        function showNotification(message, type = 'info') {
            ...
            notification.style.cssText = 'position: fixed; top: 20px; right: 20px; padding: 15px 25px; background: ' + (type === 'success' ? 'var(--primary-gradient)' : type === 'error' ? 'var(--secondary-gradient)' : 'var(--accent-gradient)') + '; color: white; border-radius: 10px; box-shadow: 0 10px 30px rgba(0,0,0,0.3); z-index: 1000; display: flex; align-items: center; gap: 10px; animation: slideInRight 0.3s ease; backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.2); max-width: 400px;';
...
app.get('/sw.js', (req, res) => {
    ...
    res.send(`const CACHE_NAME = 'cloud-movies-v2';
